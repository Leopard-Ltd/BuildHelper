...
public class BlueprintWorkFlowAndroid
{
    ...
    private async Task UploadToGithub(BlueprintWorkFlowData data)
    {
        ...
        if (string.IsNullOrEmpty(data.githubCdnData.gitExistPath))
        {
            if (!Directory.Exists(gitFolderPath))
            {
                ...
                await CommonServices.RunTerminalCommandAsync(gitCloneCommand);
                CommonServices.LogMessage($"Clone git repository to {gitFolderPath}");
            }
            else
            {
                await CommonServices.RunTerminalCommandAsync("git reset --h", gitFolderPath);
        ...
        if (hasChange)
        {
            ...
            CommonServices.LogMessage("Git changes committed and pushed.");
        }
        else
        {
            CommonServices.LogMessage("No git changes detected. Skip commit and push.");
        }
        ...
        if (string.IsNullOrEmpty(data.githubCdnData.gitExistPath))
        {
            if (Directory.Exists(gitFolderPath))
            {
                ...
                CommonServices.LogMessage($"Deleted git folder: {gitFolderPath}");
            }
            else
            {
                CommonServices.LogMessage($"Git folder not found: {gitFolderPath}");
            }
        }
    }
    ...
    private void ForceDeleteDirectory(string targetDir)
    {
        ...
        foreach (var file in files)
        {
            ...
            {
                CommonServices.LogMessage($"Cannot delete file: {file}\n{ex}");
            }
        }
        ...
        foreach (var dir in dirs.Reverse())
        {
            ...
            {
                CommonServices.LogMessage($"Cannot delete sub-folder: {dir}\n{ex}");
            }
        }
    ...
    private bool MoveALlCCdataToGitHubCCd(BlueprintWorkFlowData data, string gitRootFolder)
    {
        ...
        foreach (var file in ccdFiles)
        {
            ...
            CommonServices.LogMessage($"Copied {file} to {destinationPath}");
        }
    ...
    private void ZipFolderSafe(string folderPath, string zipPath)
    {
        ...
        {
            CommonServices.LogMessage($"ZIP failed: {ex.Message}");
        }
        finally
        {
        ...
        CommonServices.LogMessage($"Zipped folder to: {zipPath}");
    }
    ...
    private async Task ProcessAddressable(BlueprintWorkFlowData data)
    {
        ...
        {
            CommonServices.LogMessage($"Profile '{profileName}' already exists, ignore created new.");
        }
        else
        {
            profileId = profileSettings.AddProfile(profileName, copiedFromProfileId);
            CommonServices.LogMessage($"Created new Addressable profile: {profileName} (ID: {profileId})");
        }
    ...
    private BlueprintWorkFlowData TestingGetBlueprintWorkFlowData()
    {
        ...
        CommonServices.LogMessage($"{JsonUtility.ToJson(data)}");
